...
function Options:draw()
	...
	love.graphics.print("Audio:", 250, 300)
	love.graphics.print("Level:", 250, 375)
...
function Game:draw()
	...
	for i=1,size do
		love.graphics.printf(self.horizontal[i],
					gx+(50*i)-50,
					gy-((self.horizontal[i]:len()/2) * font["default"]:getHeight() * font["default"]:getLineHeight()) + 20,
					50, "center")
		love.graphics.printf(self.vertical[i], 0, gy+(50*i)-15, gx, "right")
	end
	...
	if self.win ~= -999 then
		...
		if self.win > 0 then
			...
			love.graphics.setColor(unpack(color["text"]))
			love.graphics.setFont(font["default"])
			love.graphics.printf("You completed a level " .. size .. " puzzle in: \n" .. string.format("%.2f", self.time) .. " seconds", 0, 200, love.graphics.getWidth(), "center")
			-- Buttons
